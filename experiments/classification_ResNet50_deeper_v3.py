"""
Experiment: ResNet50 Deeper v3 Classification

TRUE CNN customization: Added convolutional blocks after layer3 ONLY.
Single FC head (like Baseline) - isolating impact of late-layer conv blocks.
All layers trainable (NO freezing) - following teacher's methodology requirements.

Purpose: Compare with V1/V2 to understand which layer benefits most from additional conv blocks.

Usage:
    python experiments/classification_ResNet50_deeper_v3.py [--pretrained True/False] [--dataAugmentation none/standard/enhanced]
...
from src.models.ResNet50ClassifierModel import ResNet50Classifier, CUSTOMIZED_DEEPER_V3_CONFIG
from src.training.ResNet50_trainer import ClassificationTrainer, TRAINING_CONFIG_DEEPER_V3
...
def main():
    """Run Customized Deeper v3 Experiment."""
    
    # Parse command line arguments
    parser = argparse.ArgumentParser(description='ResNet50 Deeper v3 Classification Experiment')
    ...
    print("=" * 80)
    print("EXPERIMENT: ResNet50 Deeper v3")
    print("=" * 80)
    ...
    trainer_temp = ClassificationTrainer(model, config=TRAINING_CONFIG_DEEPER_V3)
    trainer_temp.print_model_summary()
    
    # Step 3: Train
    print("\n[3/5] Training...")
    trainer = trainer_temp  # Reuse the trainer we created for model summary
    criterion = torch.nn.CrossEntropyLoss(label_smoothing=TRAINING_CONFIG_DEEPER_V3.label_smoothing)
    ...
    evaluator.generate_experiment_summary(
        ...
        training_config=TRAINING_CONFIG_DEEPER_V3,
