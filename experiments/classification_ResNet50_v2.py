"""
Experiment: ResNet50 Customized v2 Classification

TRUE CNN customization: Added convolutional blocks after layer2.
Enhanced multi-layer FC head with BatchNorm.
...
from src.training.ResNet50_trainer import ClassificationTrainer, TRAINING_CONFIG_V2
...
def main():
    ...
    print("=" * 80)
    print("EXPERIMENT: ResNet50 Customized v2")
    print("=" * 80)
    ...
    model_config = CUSTOMIZED_V2_CONFIG.copy()
    ...
    trainer_temp = ClassificationTrainer(model, config=TRAINING_CONFIG_V2)
    trainer_temp.print_model_summary()
    
    # Step 3: Train
    print("\n[3/5] Training...")
    trainer = trainer_temp  # Reuse the trainer we created for model summary
    criterion = torch.nn.CrossEntropyLoss(label_smoothing=TRAINING_CONFIG_V2.label_smoothing)
    ...
    evaluator.generate_experiment_summary(
        ...
        training_config=TRAINING_CONFIG_V2,
