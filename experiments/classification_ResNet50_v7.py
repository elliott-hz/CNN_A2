"""
Experiment: ResNet50 Customized v7 Classification
...
Purpose: Compare with V5/V6 to understand which layer benefits most from additional conv blocks.

Usage:
    python experiments/classification_ResNet50_v7.py [--pretrained True/False] [--dataAugmentation none/standard/enhanced]
...
def main():
    """Run Customized v7 Experiment."""
    
    # Parse command line arguments
    parser = argparse.ArgumentParser(description='ResNet50 Customized v7 Classification Experiment')
    ...
    print("=" * 80)
    print("EXPERIMENT: ResNet50 Customized v7")
    print("=" * 80)
    ...
    model_config = CUSTOMIZED_V7_CONFIG.copy()
    ...
    trainer_temp = ClassificationTrainer(model, config=TRAINING_CONFIG_V7)
    trainer_temp.print_model_summary()
    
    # Step 3: Train
    print("\n[3/5] Training...")
    trainer = trainer_temp  # Reuse the trainer we created for model summary
    criterion = torch.nn.CrossEntropyLoss(label_smoothing=TRAINING_CONFIG_V7.label_smoothing)
    ...
    evaluator.generate_experiment_summary(
        ...
        training_config=TRAINING_CONFIG_V7,
