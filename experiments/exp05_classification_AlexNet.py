"""
Experiment 05: Classification Model - AlexNet

This experiment trains an AlexNet model for dog emotion classification.
...
- ~60M parameters (much smaller than ResNet50's ~25M)
"""
...
from src.models.classification_model import AlexNetClassifier, ALEXNET_BASELINE_CONFIG
...
def main():
    """Run Experiment 05: AlexNet Classification."""
    
    # Parse command line arguments
    parser = argparse.ArgumentParser(description='Experiment 05: AlexNet Classification')
    ...
    experiment_name = "exp05_classification_AlexNet"
    logger = setup_logger(experiment_name)
    
    logger.info("=" * 80)
    logger.info(f"STARTING EXPERIMENT: {experiment_name}")
    logger.info("Model: AlexNet (5 conv layers + 3 FC layers)")
    logger.info("=" * 80)
    ...
    model = AlexNetClassifier(model_config)
