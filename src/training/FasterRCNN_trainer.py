...
FASTERRCNN_V1_CONFIG = {
    ...
    'epochs': 120,                # 300+ epochs as required by instructor
    'optimizer': 'adam',          # Adam optimizer for stable convergence
    'weight_decay': 1e-4,         # Standard regularization
    'patience': 15,               # Extended patience for long training (early stopping after 50 epochs no improvement)
}
...
FASTERRCNN_V2_CONFIG = {
    ...
    'epochs': 130,                # More epochs for deeper model to converge
    'optimizer': 'adam',
    'weight_decay': 5e-4,         # Higher weight decay to prevent overfitting in deeper model
    'patience': 20,               # Longer patience for deeper model (needs more time to stabilize)
}
...
FASTERRCNN_V3_CONFIG = {
    ...
    'epochs': 110,                # Simpler model may converge faster
    'optimizer': 'adam',
    'weight_decay': 1e-4,         # Standard regularization
    'patience': 15,               # Moderate patience (simpler model stabilizes quicker)
}
